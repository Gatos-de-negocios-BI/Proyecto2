...
with DAG(
    ...
    postgres_conn_id="poblacion_etl", # Nótese que es el mismo ID definido en la conexión Postgres de la interfaz de Airflow
    sql= crear_tablas()
    )
    ...
    with TaskGroup('poblar_tablas') as poblar_tablas_dimensiones:
        ...
        poblar_city = PostgresOperator(
            task_id="poblar_anio",
            postgres_conn_id='poblacion_etl',
        ...
        poblar_customer = PostgresOperator(
            ...
            postgres_conn_id='poblacion_etl',
            sql=insert_query_subcategoria(csv_path ="subcategoria")
        )


    # task: 4 poblar la tabla de hechos
    poblar_fact_order = PostgresOperator(
            task_id="construir_poblacion",
            postgres_conn_id='poblacion_etl',
            sql=insert_query_poblacion(csv_path = "DemografiaYPoblacion_")
    )
