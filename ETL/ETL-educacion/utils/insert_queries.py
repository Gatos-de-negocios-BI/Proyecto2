...
def insert_query_educacion(**kwargs):
    insert = f"INSERT INTO educacion (educacion_Key,Subcategoria_Key,UnidadDeMedida_Key, Indicador_Key, Anio_Key, Municipio_Key, datoNumerico) VALUES "
    insertQuery = ""
    # Es necesario colocar este try porque airflow comprueba el funcionamiento de las tareas en paralelo y al correr el DAG no existe el archivo dimension_city. Deben colocar try y except en todas las funciones de insert
    try:
        dataframe =cargar_datos(kwargs['csv_path'])
        for index, row in dataframe.iterrows():
            insertQuery += insert + f"({row.educacion_key},{row.Subcategoría},{row['Unidad de Medida']},{row.Indicador},{row.Año}, {row['Código Entidad']},{row['Dato Numérico']});\n"
        return insertQuery
    except:
        return ""