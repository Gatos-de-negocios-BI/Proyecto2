...
with DAG(
    ...
    postgres_conn_id="salud_etl", # Nótese que es el mismo ID definido en la conexión Postgres de la interfaz de Airflow
    sql= crear_tablas()
    )
    ...
    with TaskGroup('poblar_tablas') as poblar_tablas_dimensiones:
        ...
        poblar_customer = PostgresOperator(
            ...
            postgres_conn_id='salud_etl',
            sql=insert_query_subcategoria(csv_path ="subcategorias_salud")
        )
        ...
        poblar_customer = PostgresOperator(
            ...
            postgres_conn_id='salud_etl',
            sql=insert_query_unidadDeMedida(csv_path ="unidades_salud")
        )
        ...
        poblar_customer = PostgresOperator(
            ...
            postgres_conn_id='salud_etl',
            sql=insert_query_indicador(csv_path ="indicador_salud")
        )


    # task: 4 poblar la tabla de hechos
    poblar_fact_order = PostgresOperator(
            task_id="construir_salud",
            postgres_conn_id='salud_etl',
            sql=insert_query_salud(csv_path = "Salud_")
    )
